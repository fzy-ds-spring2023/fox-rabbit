...
HUNGER = 10 # How many seasons a fox can go without eating before it dies
MOVEMENT = 2 # How far a fox can move in one season
...
class Rabbit:
    ...
    def move(self):
        ...
        if WRAP:
            self.x = (self.x + rnd.randint(-1,1)) % SIZE
            self.y = (self.y + rnd.randint(-1,1)) % SIZE
        else:
            self.x = min(SIZE-1, max(0, (self.x + rnd.randint(-1,1))))
            self.y = min(SIZE-1, max(0, (self.y + rnd.randint(-1,1))))
            
class Fox:
    """ An orange-furred mammal roaming a field in search of rabbit to eat.
    ...
    def move(self):
        ...
        if WRAP:
            self.x = (self.x + rnd.randint(-MOVEMENT,MOVEMENT)) % SIZE
            self.y = (self.y + rnd.randint(-MOVEMENT,MOVEMENT)) % SIZE
        else:
            self.x = min(SIZE-1, max(0, (self.x + rnd.randint(-MOVEMENT,MOVEMENT))))
            self.y = min(SIZE-1, max(0, (self.y + rnd.randint(-MOVEMENT,MOVEMENT))))
...
class Field:
    ...
    def eat(self):
        ...
        for fox in self.foxes:
            fox.eat(self.get_rabbits[fox.x,fox.y])
            self.rabbits = [r for r in self.rabbits if r.x != fox.x and r.y != fox.y]
